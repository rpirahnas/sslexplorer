...
import com.sslexplorer.properties.ProfilesListDataSource;
...
import com.sslexplorer.setup.SystemInformationProvider;
import com.sslexplorer.setup.SystemInformationRegistry;
...
public class DefaultLogonController implements LogonController {
	...
	public AccountLock checkForAccountLock(String username, String realmName) throws SecurityErrorException,
		...
		AccountLock lock = "true".equals(SystemProperties.get("sslexplorer.recoveryMode", "false")) ? null
	...
	// // Get the current lock (if any)
	// AccountLock lock =
	// "true".equals(SystemProperties.get("sslexplorer.recoveryMode", "false")) ?
	// null
	// : (AccountLock) lockedUsers.get(username);
	// // If the user is currently locked, check if the lock has expired yeet
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.sslexplorer.security.LogonController#applyMenuItemChanges(javax.servlet.http.HttpServletRequest)
	 */
    	                ...
    	                for(Iterator i = sessions.values().iterator(); i.hasNext(); ) {
    	                    ...
    	                    if(sesh.getHttpSession() != null) {
    	                        try {
    	                            ...
    	                            log.error("------------------------------------------------------");
    	                            log.error("An SSL-Explorer session that is attached to an");
