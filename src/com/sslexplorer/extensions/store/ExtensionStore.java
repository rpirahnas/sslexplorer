...
public class ExtensionStore {
		...
		for (ExtensionBundle bundle : extensionBundlesList) {
			if (bundle.getInstaller().getOpCount() > 0 && bundle.getStatus().isStartedOrActivated()) {
				...
				if (force || ver.equals("") || !ver.equals(bundle.getVersion().toString())) {
				    if(force) {
                        log.info("Will run installer for " + bundle.getId() + " because it has been forced by the sslexplorer.foreceInstallers property.");
				    }
				    else if (ver.equals("")) {
	...
	private static URL getDownloadURL(String id, String version) {
		try {
            String location = SystemProperties.get("sslexplorer.downloadableApplicationStore.location", ExtensionStore.HTTP_3SP_COM_APPSTORE);
			...
			try {
				...
				log.error("Invalid downloadable extension location specified in system property sslexplorer.downloadableApplicationStore.location, '" + SystemProperties.get("sslexplorer.downloadableApplicationStore.location")
	...
	public static URL getStoreDownloadURL(String appStoreLocation, VersionInfo.Version version) {
		try {
			String location = SystemProperties.get("sslexplorer.downloadableApplicationStore.location", appStoreLocation);
			...
			try {
				...
				log.error("Invalid downloadable extension store location specified in system property sslexplorer.downloadableApplicationStore.location, '" + SystemProperties.get("sslexplorer.downloadableApplicationStore.location")
					+ "'. Must be either a URL or the file path of the store descriptor file.");
			}
		}
		return null;
    ...
    public static VersionInfo.Version getWorkingVersion() {
        VersionInfo.Version version = new VersionInfo.Version(SystemProperties.get("sslexplorer.forceVersion", ContextHolder.getContext().getVersion().toString()));
        return version;
    }
		...
		while (t.hasMoreTokens()) {
			...
			if (ext.equalsIgnoreCase("all")) {
				...
				File[] dirs = f.listFiles(new FileFilter() {
					public boolean accept(File pathname) {
						File f = new File(pathname, "extensions");
						return f.exists() && f.isDirectory() && pathname.getName().indexOf("sslexplorer-community-") != -1;
					}
				});
				for (int i = 0; dirs != null && i < dirs.length; i++) {
					devExtensions.add(dirs[i].getName());
