...
public class CoreRequestProcessor extends TilesRequestProcessor {
	...
	private void setDefaultLocale(HttpServletRequest request, HttpSession session) {
		...
		try {
            if (Util.isNullOrTrimmedBlank(SystemProperties.get("sslexplorer.defaultLanguage", ""))) {
    		    Cookie[] c = request.getCookies();
    		    if (c != null) {
    		        for (int i = 0; i < c.length; i++) {
    		            if (c[i].getName().equals(SystemProperties.get("sslexplorer.cookie", "SSLX_SSESHID") + "_LANG")) {
                ...
                defaultLocale = SystemProperties.get("sslexplorer.defaultLanguage", "");
            }
		    Locale locale;
    ...
    private static boolean isRunningUnitTests() {
        String isRunningUnitTests = SystemProperties.get("sslexplorer.testing", "false");
