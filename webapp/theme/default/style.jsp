...
<jsp:directive.page import="com.sslexplorer.core.BrowserChecker"/>
...
<%@page import="com.sslexplorer.extensions.ExtensionBundle"%>
...
<%
	// Background color
	String c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.background", "");
	if(!c.equals("")) {
		out.println("body { background-color: " + c + "; }");		
	}
	
	// Foreground color
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.foreground", "");
	if(!c.equals("")) {
		out.println("body { color: " + c + "; }");		
	}
	
	// Font
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.font", "");
	if(!c.equals("")) {
	...
	// Header color
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.header.background", "");
	if(!c.equals("")) {
		out.println("div#component_pageHeader { background-color: " + c + "; }");		
	}
	
	// Sidebar background color
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.sideBar.background", "");
	if(!c.equals("")) {
		out.println("div#component_navmenu, #layout_footer, #layout_menus { background-color: " + c + "; }");		
	}
	
	// Sidebar foreground color
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.sideBar.foreground", "");
	if(!c.equals("")) {
		out.println("div#component_navmenu a.deselected, div#component_navmenu, #layout_footer, #layout_menus { color: " + c + "; }");			
	}
	
	// Sidebar active color
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.sideBar.active.foreground", "");
	if(!c.equals("")) {
		out.println("div#component_navmenu a.selected, #layout_footer #component_logonStatus { color: " + c + "; }");			
	}
	
	// Navmode background color
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.navmode.background", "");
	if(!c.equals("")) {
		out.println("div#component_navmode { background-color: " + c + "; }");		
	}
	
	// Navmode foreground color
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.navmode.foreground", "");
	if(!c.equals("")) {
		out.println("div#component_navmode { color: " + c + "; }");		
	}
	
	// Panel background color
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.panel.background", "");
	if(!c.equals("")) {
	...
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.panel.background.darker", "");
	if(!c.equals("")) {
		out.println(".tabHeadings a:link, .tabHeadings a:visited { background-color: " + c + "; }");
	}
	
	// Panel foreground color
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.panel.foreground", "");
	if(!c.equals("")) {
	...
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.panel.foreground.darker", "");
	if(!c.equals("")) {
		out.println(".tabHeadings a:link, .tabHeadings a:visited { color: " + c + "; }");
	}
	
	// Panel border color
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.panel.border", "");
	if(!c.equals("")) {
		out.println(".tabContent, .dialog, .dialog_content,.titled_dialog_content { border: " + c + "; }");			
	}
	
	// Panel rounded corners (moz. only)
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.panel.border.rounded", "true");
	if(c.equals("true")) {
	...
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.table.header.background", "");
	if(!c.equals("")) {
		out.println(".resource_table thead tr, .resource_table tfoot tr { background-color: " + c + "; }");
	}
	
	// Table header foreground
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.table.header.foreground", "");
	if(!c.equals("")) {
	...
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.table.highlight.background", "");
	if(!c.equals("")) {
		out.println(".resource_table tr.highlight { background-color: " + c + "; }");
	}
	
	// Table highlight foreground
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.table.highlight.foreground", "");
	if(!c.equals("")) {
	}
	
	// Table Lowlight background
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.table.lowlight.background", "");
	if(!c.equals("")) {
		out.println(".resource_table tr.lowlight { background-color: " + c + "; }");
	}
	
	// Table Lowlight foreground
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.table.lowlight.foreground", "");
	if(!c.equals("")) {
	}
	
	// Button background
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.button.background", "");
	if(!c.equals("")) {
		out.println("input.cancel, input.close, input.configure, input.exit, input.finish, input.multiAdd, input.multiRemove," +
					"input.new, input.no, input.ok, input.next, input.previous, input.reset, input.retry, input.save, input.yes," +
	...
	// Button foreground
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.button.foreground", "");
	if(!c.equals("")) {
		out.println("input.cancel, input.close, input.configure, input.exit, input.finish, input.multiAdd, input.multiRemove," +
					"input.new, input.no, input.ok, input.next, input.previous, input.reset, input.retry, input.save, input.yes," +
	...
	// Button border
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.button.border", "");
	if(!c.equals("")) {
		out.println("input.cancel, input.close, input.configure, input.exit, input.finish, input.multiAdd, input.multiRemove," +
					"input.new, input.no, input.ok, input.next, input.previous, input.reset, input.retry, input.save, input.yes," +
	...
	// Link foreground
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.link.foreground", "");
	if(!c.equals("")) {
		out.println("a:link, a:visited, a:hover, a:active { color: " + c + "; }");
	}
	
	// Link font
	c = SystemProperties.get("sslexplorer.defaultTheme.userDefinedStyles.link.font", "");
	if(!c.equals("")) {
